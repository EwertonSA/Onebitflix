...
function ensureAuth(req, res, next) {
    ...
    jwtService_1.jwtService.verifyToken(token, (err, decoded) => __awaiter(this, void 0, void 0, function* () {
        ...
        const jwtPayload = decoded; // Assertiva de tipo
        const user = yield userService_1.userService.findByEmail(jwtPayload.email);
        req.user = user;
        next();
    }));
}
...
function ensureAuthViaQuery(req, res, next) {
    ...
    if (typeof token !== 'string')
        return res.status(400).json({ message: 'O paramentro token deve ser do tipo string' });
    jwtService_1.jwtService.verifyToken(token, (err, decoded) => __awaiter(this, void 0, void 0, function* () {
        if (err || typeof decoded === "undefined")
            return res.status(401).json({ message: "Não autorizado: token inválido" });
